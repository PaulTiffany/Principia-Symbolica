...
\begin{description}[leftmargin=0pt, itemsep=6pt]
    ...
    \textit{Derived, not imported.}
    Free-energy minimization is a theorem in PS, derived from drift--reflection dynamics over $\manifold_O$
    (cf.~\S\ref{sec:appD_ps_and_free_energy_principle}, Appendix~C).
    The FEP is a recoverable consequence; it is not postulated.

    \item[\textbf{Statistical Thermodynamics \& Information Theory}] \hfill \\
    \textit{Extended, not borrowed.}
    $\entropy$ and $\freeenergy$ are derived as theorems over observer-relative states via $K_O$ acting on $\manifold_O$;
    the derivation runs through the observer's perceptual kernel, not through analogy with physical microstates
...
(LLMs) often behave as unwitting SRV practitioners.
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Framework for AGI Development:} PS principles apply directly to structuring AGI development, treating iterative refinement, confidence management, and ethical constraints as symbolic thermodynamic processes governed by $\drift$, $\reflect$, and $\freeenergy$.
\end{itemize}
