...
Let \(\mathcal{S} = (M, g, D, R, \rho)\) be any symbolic system (cf.~\ref{definition:bk1_symbolic_manifold}) that exhibits:
...
\begin{itemize}
    ...
    \item Fundamentally, PS defines Drift (\(\drift\)) as a primordial aspect of symbolic systems (Ax.~\ref{axiom:bk1_axiomata_prima}, "Existence is not," implies change/drift as fundamental to becoming). A system without effective Drift is not a complete symbolic system as per PS.
\end{itemize}
...
\subsection[Elimination.3 Case C: Neither Horizon-Effect]{Elimination.3 Case C: Neither Generative nor Constraining/Dissipative Horizon-Effect} \label{subsec:appC_case_c_neither_generative_nor_constraining_dissipative}
...
This proof underscores that the Generative and Constraining/Dissipative horizon-effects are not merely additive features but are co-constitutive of any viable symbolic system. One cannot exist meaningfully without the other if the system is to maintain identity while evolving under observation. Drift (cf.~\ref{definition:bk1_drift_field}) necessitates Reflection for coherence; Reflection requires Drift for dynamism. Their interplay — formally enacted as Symbolic Reflexive Validation (cf.~\ref{definition:bk7_symbolic_reflexive_validation_srv}) — at the boundary defined by the Bounded Observer, is the crucible of symbolic existence and becoming. This duality is fundamental, echoing throughout the structure of Principia Symbolica, from the emergence of manifolds to the dynamics of cognitive freedom.
\end{scholium}
\clearpage
...
Boundedness (PS-C1, \ref{axiom:appC_psc1}) together with finite additivity
(PS-C3, \ref{axiom:appC_psc3}, established as
Thm.~\ref{theorem:appC_orthogonal_additivity}) imply that for fixed $\psi$,
...
Finite additivity is Thm.~\ref{theorem:appC_orthogonal_additivity} (PS-C3).
...
By Axiom~\ref{axiom:appC_psc2} (unitary covariance),
$\mathcal{C}_{\Obs}(U\tilde\psi_{\Obs}, U\Pi U^\dagger) = \mathcal{C}_{\Obs}(\tilde\psi_{\Obs}, \Pi)$.
By Axiom~\ref{axiom:appC_psc4} (ray invariance), replacing $\tilde\psi_{\Obs}$ by
$U\tilde\psi_{\Obs}$ leaves $\mathcal{C}_{\Obs}$ unchanged up to a global phase, which
is absorbed. Hence $\mu_\psi(U\Pi U^\dagger)
= \mathcal{C}_{\Obs}(\tilde\psi_{\Obs}, U\Pi U^\dagger)
...
\]
This result follows from the Bounded Observer structure (cf.~\ref{definition:bk4_bounded_observer}) applied in the quantum regime.
\end{theorem}
...
Embed $\mathbb{C}^2$ into $\mathbb{C}^3$ (cf.~\ref{definition:appC_frame_space}) to apply Gleason's result.
\end{corollary}

\begin{corollary}[Mixed states]
\label{corollary:appC_mixed_states}
For density operator $\rho = \sum p_i |\psi_i\rangle\langle\psi_i|$ (cf.~\ref{definition:appC_frame_space}),
\[
...
The fundamental irreversibility established in Theorem~\ref{theorem:appC_fundamental_irreversibility_final} generates a directed Arrow of Time. The system's evolution, governed by the minimization of Symbolic Free Energy \(\freeenergy\) (cf.~\ref{definition:bk2_symbolic_free_energy}), proceeds along a path. Because every step on this path is recorded in the observer's history and cannot be erased without metabolic cost, the path itself is directional. The direction of decreasing \(\freeenergy\) becomes the direction of "forward" in time.
\end{corollary}
...
Let the symbolic system evolve under the Lagrangian (cf.~\ref{definition:appC_lagrangian_potential}):
\[
...
Under symbolic curvature and recursive constraint (cf.~\ref{definition:bk2_symbolic_free_energy}), the minimum sustainable growth rate is $\varphi$.
\end{theorem}
...
Assume $\frac{dC}{dt} = \delta/C(t)$ (cf.~\ref{theorem:appC_phi_min_growth}). Then:
\[
...
Among all $\lambda$ (cf.~\ref{definition:appC_complexity_entropy_tradeof}), $\varphi$ minimizes symbolic inefficiency:
\[
...
Under the constraint that symbolic evolution (cf.~\ref{definition:appC_symbolic_operator_assumptions}) preserves a two-dimensional state space $(s_1, s_2)$ where $s_1$ represents current symbolic complexity and $s_2$ represents previous symbolic complexity, the composed operator $T_\lambda$ has matrix representation:
\[
...
The symbolic evolution rule (cf.~\ref{lemma:appC_matrix_representation_symbolic_operators}) is:
\begin{align}
...
\leavevmode\newline
The golden ratio $\varphi$ emerges as the dominant eigenvalue of $M_\lambda$
when $\lambda$ satisfies the recursive emergence condition
(cf.~Def.~\ref{definition:bk1_stage_composite_operator}).
\end{theorem}
...
The characteristic polynomial of $M_\lambda$ (cf.~\ref{lemma:appC_matrix_representation_symbolic_operators}) is:
\[
...
For $M_\varphi$ (Def.~\ref{lemma:appC_matrix_representation_symbolic_operators}) with $\varphi$ satisfying $\varphi^2 = \varphi + 1$, the matrix powers generate Fibonacci-like sequences.
\end{lemma}
...
We prove by induction (cf.~\ref{lemma:appC_fibonacci_structure_matrix_powers}) that $M_\varphi^n = \begin{bmatrix} a_n & -a_{n-1} \\ a_{n-1} & a_{n-2} \end{bmatrix}$ where $a_n$ satisfies the recurrence $a_{n+1} = \varphi a_n - a_{n-1}$.
...
For the curvature $\kappa$ (Def.~\ref{definition:appC_symbolic_curvature_function}), unconstrained minimization: $\frac{d\kappa}{d\theta} = 1 - \frac{1}{\theta^2} = 0$ gives $\theta = 1$ with $\kappa(1) = 2$. However, this represents the degenerate case where forward drift equals reflective pull, resulting in symbolic stagnation.
...
The curvature at this point is $\kappa(\varphi) = \varphi + \frac{1}{\varphi} = \varphi + (2 - \varphi) = 2$, but this represents the 	extbf{sustained} minimum rather than the degenerate minimum.
\end{proof}
...
Matrix approach (cf.~\ref{theorem:appC_unified_recursive_fixed_point}): The recursive emergence condition requires $\lambda = 1 + 1/\lambda$ for the growth parameter to be self-consistent with the evolution rule.
