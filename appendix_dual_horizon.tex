...
This derivation proceeds from five coherence axioms (PS-C1--C5,
\S\ref{subsec:appC_born_axioms}) to Gleason's hypotheses, and thence to
the Born Rule. The axioms PS-C1--C5 are themselves consequences of bounded
observation (Def.~\ref{definition:bk1_bounded_observer}), not ad hoc
quantum postulates: each encodes a constraint that any finite-resolution
observer necessarily satisfies. The derivation is therefore grounded in
the PS foundational framework, not in the Hilbert space structure it explains.
\end{remark}
...
The following axioms constrain the coherence functional (cf.~\ref{definition:appC_coherence_functional}) of a Bounded Observer.
...
The fixed-point equation $\lambda = 1 + 1/\lambda$ (cf.~\ref{theorem:appC_unified_recursive_fixed_point}) appears in multiple contexts within symbolic dynamics. The consistent emergence of $\varphi$ across matrix, topological, and (potentially) thermodynamic or spectral approaches suggests a deeper structural principle governing bounded symbolic systems.
\end{remark}
