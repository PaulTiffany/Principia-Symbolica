...
$D_\lambda$ induces transformations that are \emph{observable} to $\mathcal{O}$.
By Def.~\ref{definition:bk1_bounded_observer}, a transformation is observable
to $\mathcal{O}$ precisely when the perceived change lies within the resolution
threshold $\delta_\mathcal{O}$; a transformation exceeding $\delta_\mathcal{O}$
falls outside the observer's resolution kernel $\mathcal{K}_\mathcal{O}$ and is therefore unobservable (Def.~\ref{definition:bk1_bounded_observer}).
Therefore, the effective proto-drift
$\vec{D}_\lambda^{eff}(s) = D_\lambda(s) \ominus s$
(Def.~\ref{definition:bk1_proto_drift_field}) satisfies:
\[
\sup_{s \in P_{<\lambda}} \|\vec{D}_\lambda^{eff}(s)\| \leq \delta_\mathcal{O}.
\]
By Proposition~\ref{proposition:bk1_boundedness_from_drift},
$D_\lambda$ is therefore a Bounded Symbolic Approximation
(Def.~\ref{definition:bk1_bounded_symbolic_approximation}).

\textbf{For $R_\lambda$.}
$R_\lambda: P_\lambda \to P_\lambda$ is an idempotent stabilization operator
(Def.~\ref{definition:bk1_pre_geometric_operators_and_stages}).
By the same axiom, $R_\lambda$ induces observable transformations, so
$\|R_\lambda(s) - s\|_\mathcal{O} \leq \delta_\mathcal{O}$ for all
$s \in P_\lambda$.
Applying Proposition~\ref{proposition:bk1_boundedness_from_drift} to the
proto-drift $\vec{R}_\lambda^{eff}(s) = R_\lambda(s) - s$ gives the same
conclusion: $R_\lambda$ is a Bounded Symbolic Approximation.
\end{proof}
...
To a bounded observer (Def.~\ref{definition:bk1_bounded_observer}), the entire tower of emergent symbolic structures (Def.~\ref{definition:bk1_pre_geometric_operators_and_stages}) unfolds inside an invariant
...
Let $\mathcal{U}$ be a symbolic universe sustaining bounded observers within a domain $\Omega$ (cf.~Def.~\ref{definition:bk1_bounded_observer}), whose stagewise structures cohere into a categorical colimit (cf.~Proof~\ref{proof:bk1_colimit_yields_categoric_structure}). Then $\mathcal{U}$ supports reflexive emergence if and only if it possesses both:
\begin{enumerate}
...
Every stable symbolic structure $\mathcal{S}$ with reflection operator $\reflect$ (Def.~\ref{definition:bk1_reflection_operator}; cf.~Scholium~\ref{scholium:bk1_constitutive_reflex}) generates its own observer $\mathcal{O}$ such that:
\begin{align}
\mathcal{O} = \lim_{n \to \infty} \reflect^n(\mathcal{S})
\end{align}
...
\item \textbf{Reflection Necessity}: Stability demands $\reflect: \mathcal{S} \rightarrow \mathcal{S}$ to detect and correct boundary violations.
\item \textbf{Iterative Refinement}: Each application $\reflect^n(\mathcal{S})$ refines the system's self-knowledge and boundary discrimination.
\item \textbf{Observer Emergence}: The limit $\lim_{n \to \infty} \reflect^n(\mathcal{S})$ converges to the maximal self-reflective subset—the observer.
\end{enumerate}
...
Let $\mathcal{S}$ be a symbolic manifold as defined in Def.~\ref{definition:bk1_symbolic_manifold}.
The reflection operator \( R: T\mathcal{S} \rightarrow T\mathcal{S} \) is the stabilising counterpart to the drift field (Def.~\ref{definition:bk1_drift_field}), encoding the capacity for self-reference that arises necessarily from Axiom~\ref{axiom:bk1_axiomata_prima}. It is a smooth fiber-preserving map on the tangent bundle satisfying:
\begin{enumerate}
...
\begin{itemize}
    ...
    \item Reflexive identity is inherited from symbolic identity carriers (cf.~Definition~\ref{definition:bk1_let_cats_be_the_category}).
...
Let $\mathcal{I}_s$ denote the symbolic identity structure at state $s$. For reflexive identity, the system must satisfy:
\[
...
\begin{itemize}
    ...
    \item \textbf{Quantum Theory (quant-ph)}: Surprise defines measurement-induced collapse as curvature perturbation from bounded hypothesis sets; in the flat-frame limit this recovers the standard Born rule from PS axioms alone.
...
\textbf{Proof:} 
\begin{enumerate}
\item By Theorem~\ref{theorem:bk1_reflexivity_quadratic}, the system requires quadratic forms
\item By Proposition~\ref{proposition:bk1_bridge_to_geometry}, these are metric tensors $g_{ij}(x)$
...
The symbolic curvature $\kappa$ (arising from the symbolic connection of Def.~\ref{definition:bk1_symbolic_connection}) vanishes if and only if symbolic meanings are locally independent (parallel transport is path-independent).
\end{proposition}
...
Both summands are positive semi-definite: each has the form
$\langle L(\cdot), L(\cdot)\rangle_0$ for a linear map $L$, composed with an inner product. Positive-definiteness of the sum follows from the proto-stage
...
\textbf{Existence and uniqueness of $R$.}
...
$M = \varinjlim P_\lambda$, there is a unique map $R: M \to M$ such that
$R \circ \iota_\lambda = \iota_\lambda \circ R_\lambda$ for each inclusion
$\iota_\lambda: P_\lambda \hookrightarrow M$.
Smoothness of $R$ follows from Ax.~\ref{axiom:bk1_smooth_convergence}: the
$R_\lambda$ converge in $C^\infty$ on compact subsets, so $R \in C^\infty(M)$.

\textbf{Contraction property.}
Each $R_\lambda$ consolidates symbolic coherence: it maps dispersed proto-states toward
more coherent configurations, reducing $g_\lambda$-dispersion by a factor $\kappa_\lambda$.
Formally, for $x,y \in P_\lambda$,
\[
  d_\lambda(R_\lambda(x), R_\lambda(y)) \leq \kappa_\lambda \cdot d_\lambda(x,y),
\]
where $\kappa_\lambda \in (0,1)$ because $R_\lambda$ strictly decreases symbolic variance
(Def.~\ref{definition:bk1_proto_symbolic_space}). The sequence $\{\kappa_\lambda\}$ is
monotone non-increasing and bounded below by 0; let $\kappa = \lim_{\lambda \to \Omega}
\kappa_\lambda$. Since $g_\lambda \to g$ uniformly on compact sets
(Ax.~\ref{axiom:bk1_symbolic_smoothness}) and the coherence maps are isometric in the
limit, passing to the colimit gives $d(R(x),R(y)) \leq \kappa \cdot d(x,y)$ for all
$x,y \in M$. The strict inequality $\kappa < 1$ holds because each finite stage already
achieves $\kappa_\lambda < 1$ and the infimum is attained at a stage $\lambda_0 < \Omega$
with $\kappa_{\lambda_0} < 1$.
\end{proof}
\end{theorem}

\begin{corollary}[Fixed Point]
\label{corollary:bk1_fixed_point}
The reflection operator $R: M \to M$ (Def.~\ref{definition:bk1_reflection_operator}) has a unique fixed point $x^* \in M$ such that $R(x^*) = x^*$.
...
\begin{proof}[Stability via Linearized Drift--Reflection System]
\label{proof:bk1_sketch_stability_drift_reflection}
\leavevmode

The combined symbolic dynamics near $x^*$ can be approximated by linearization
of the drift--reflection system
(cf.~thm.~\ref{theorem:bk1_emergence_of_reflection_operator},
cor.~\ref{corollary:bk1_fixed_point},
thm.~\ref{theorem:bk1_emergence_of_drift_field}). Consider a dynamical system of the form:
\[
...
Then $(M, \Omega)$ constitutes a \textbf{dual-horizon symbolic manifold} (Def.~\ref{definition:bk1_symbolic_manifold}) supporting reflexive emergence. In particular, our universe satisfies the conditions of the \textbf{Dual Horizon Necessity Theorem} (Thm.~\ref{theorem:bk1_dual_horizon_necessity_theorem}), and the full PS dynamical apparatus---Hamiltonian, Fokker--Planck evolution, equilibrium, and identity carriers---is thereby instantiated on $\Omega$.
\end{theorem}
...
The proof proceeds in two stages: \emph{instantiation} (mapping cosmological data to PS hypotheses) and \emph{deduction} (invoking established theorems to derive the conclusion).

\medskip
\textbf{Stage I: Instantiation of conditions.}
...
Conditions (1)--(3) supply a symbolic universe $\mathcal{U} = (M, \Omega)$ with both a generative horizon $\mathcal{H}_G$ ($\kappa > 0$) and a dissipative horizon $\mathcal{H}_D$ ($\kappa < 0$) bounding a non-empty observer domain. By the Dual Horizon Necessity Theorem (Thm.~\ref{theorem:bk1_dual_horizon_necessity_theorem}), this is the unique horizon configuration supporting reflexive emergence: the elimination argument in Proof~\ref{proof:bk1_proof_of_dual_horizon_necessity_theorem} shows that any configuration lacking either horizon fails to achieve the complexity differential $\Delta\Phi(D, R) \geq \tau_E$.

\textbf{5. Dynamical apparatus.}\enspace
Given the dual-horizon structure on $\Omega$, the PS results chain as follows:
\begin{enumerate}
    \item[\emph{(a)}] The drift field $D$ and reflection operator $R$ emerge on $\Omega$ as limits of proto-fields (Thm.~\ref{theorem:bk1_emergence_of_drift_field}, Thm.~\ref{theorem:bk1_emergence_of_reflection_operator}), with $\nabla \cdot D > 0$ near $\mathcal{H}_G$ and $\nabla \cdot R < 0$ near $\mathcal{H}_D$ (Lemma~\ref{lemma:bk1_horizon_characterization}).
    \item[\emph{(b)}] Their interplay defines the symbolic Hamiltonian $H(x) = \kappa / (\|D(x)\| + \epsilon) + \lambda \cdot \mathrm{tr}(L_x)$ (Def.~\ref{definition:bk2_symbolic_hamiltonian}), which governs the energy landscape on $\Omega$.
...
The manifold $(M, \Omega)$ satisfies all hypotheses of the Dual Horizon Necessity Theorem, and the deductive chain (a)--(e) instantiates the full PS dynamical apparatus on $\Omega$. Reflexive emergence is not merely compatible with our universe's causal architecture---it is entailed by it, conditional on the empirical conditions (1)--(3).
\end{proof}
\begin{remark}
