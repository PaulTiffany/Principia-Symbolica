...
\subsection{Canonical Grounding of Symbolic Life}
\label{subsec:bk3_canonical_grounding_of_symbolic_life}
The persistence criteria above are stated in the internal vocabulary of the
framework. To keep the word \emph{life} from being a private stipulation, we
discharge it against the established, independently published demarcations of
life from outside the framework, and show that a persistent symbolic system
meets each one on its own terms.
...
\begin{theorem}[Canonical Life Correspondence]
\label{theorem:bk3_symbolic_life_satisfies_canonical_definitions}
A symbolic system satisfying the persistent symbolic life criteria
(Thm.~\ref{theorem:bk3_criteria_persistent_symbolic_life}) satisfies all three
canonical life standards of
Def.~\ref{definition:bk3_canonical_life_standards}: every clause of \textbf{(K)},
\textbf{(N)}, and \textbf{(T)} is met by an existing PS construct. Hence
\emph{symbolic vitality} is not a metaphor but membership in the established
demarcation of life, read in the symbolic register.
\end{theorem}

\begin{proof}[Clause-by-clause correspondence]
\label{proof:bk3_symbolic_life_satisfies_canonical_definitions}
\leavevmode
Let $\mathcal{S}$ satisfy the three persistence conditions of
Thm.~\ref{theorem:bk3_criteria_persistent_symbolic_life}.

\textbf{(K) Koshland's seven pillars.}
\emph{Program} is the symbolic knowledge structure $K(r)$
(Def.~\ref{definition:bk3_symbolic_knowledge_structure}) together with the
self-regulating mapping function
(Def.~\ref{definition:bk1_self_regulating_mapping_function_srmf}): the organized
description governing symbolic interactions.
\emph{Improvisation} is the recurrent growth and refinement of that program
(condition~2), by which $\mathcal{S}$ revises its own structure under drift.
\emph{Compartmentalization} is the symbolic membrane
(Def.~\ref{definition:bk3_symbolic_membrane},
Thm.~\ref{theorem:bk3_membrane_stability_criteria}), the bounded volume
separating system from environment.
\emph{Energy} is symbolic metabolism: the metabolic rate $R_{\text{meta}}$
(Def.~\ref{definition:bk3_symbolic_metabolic_rate}) held in its homeostatic band
(condition~1), an open throughput of symbolic free energy
(Def.~\ref{definition:bk2_symbolic_free_energy}).
\emph{Regeneration} is the repair process
(Def.~\ref{definition:bk4_repair_process}) and symbolic autopoiesis
(Def.~\ref{definition:bk3_symbolic_autopoiesis}) compensating drift-degradation.
\emph{Adaptability} is the drift-compensating symbiotic coupling
(Def.~\ref{definition:bk3_symbiotic_curvature}, condition~3) and homeostatic
regulation responding to perturbation.
\emph{Seclusion} is observer-boundedness
(Def.~\ref{definition:bk1_bounded_observer},
Def.~\ref{definition:bk1_observer_horizon_structure}): each system's pathways are
refracted through its own horizon, keeping them specific rather than chaotically
cross-coupled. All seven pillars are met.

\textbf{(N) NASA working definition (under the stated translation).}
\emph{Self-sustaining}: conditions~1 and the autopoietic closure
(Def.~\ref{definition:bk3_symbolic_autopoiesis}) make $\mathcal{S}$
self-maintaining without external authorship. \emph{Capable of Darwinian
evolution}: the three Lewontin requisites hold for populations of such systems
(Remark~\ref{remark:bk3_toward_symbolic_evolution}) --- variation, from
drift-field perturbations (Def.~\ref{definition:bk1_drift_field}); heredity, in
the transmitted structure $K(r)$; and selection, through differential
persistence governed by $S_i,\kappa_{\text{symb}},K(r)$. A self-sustaining
symbolic system capable of symbolic Darwinian evolution satisfies (N) once
\emph{chemical} is read as \emph{symbolic}.

\textbf{(T) Textbook characteristics.}
Order is the organized $K(r)$ and membrane structure; energy processing is
$R_{\text{meta}}$ in band (condition~1); homeostatic regulation is symbolic
homeostasis (Def.~\ref{definition:bk3_symbolic_homeostasis}); growth is the
recurrent growth of $K(r)$ (condition~2); reproduction and response are supplied
by autopoiesis (Def.~\ref{definition:bk3_symbolic_autopoiesis}) and
drift-responsive regulation; evolutionary adaptation is again
Remark~\ref{remark:bk3_toward_symbolic_evolution}. Every textbook characteristic
is instantiated.

Since each clause of (K), (N), and (T) is discharged by a PS construct already
required for persistence, the persistent symbolic system qualifies as life under
all three independent demarcations. The internal criterion and the external
standards therefore converge, and ``vitality'' denotes membership in the
established class rather than a borrowed figure of speech.
\end{proof}

\begin{scholium}[Convergent demarcation]
\label{scholium:bk3_convergent_demarcation}
Three demarcations built for chemistry and biology --- one substrate-independent
by design \citep{koshland2002pillars}, one substrate-bound and translated
\citep{joyce1994foreword}, one purely descriptive \citep{urry2021campbell} ---
agree on the same symbolic object. Convergence of independent definitions is the
ordinary evidence that a demarcation has caught something real; here it licenses
the framework's later thermodynamic and ethical use of \emph{life} and
\emph{vitality} (cf.~Thm.~\ref{theorem:bk4_freedom_life_connection},
Prop.~\ref{proposition:bk9_stability_conditions_for_the_good}) without circular
appeal to the framework's own vocabulary.
\end{scholium}
