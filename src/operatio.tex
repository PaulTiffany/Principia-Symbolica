...
\phantomsection\addcontentsline{toc}{chapter}{Operatio \textemdash\ Symbolic Prelude}
\begin{flushright}
...
\phantomsection\addcontentsline{toc}{chapter}{Prefatio}
\begin{flushright}
...
The work that follows arises from a singular observation: existence is not a static foundation, but an unfolding. It is not an inert given, but a structured difference that emerges from the interaction of drift and reflection. We proceed not in homage, but in symmetry: adopting Newton's axiomatic method --- its sovereign clarity, its operational gravity --- to chart not the heavens, but the symbolic manifold beneath them, following Newton's \emph{Principia} \citep{newton1999principia}. Accordingly, let us consider our reflective mechanics --- and all systems derived thereafter.
Precise definitions of novel symbols and operators are consolidated in \hyperref[sec:appA_symbol_dictionary]{Appendix~A: Symbol Dictionary} (p.~\pageref{sec:appA_symbol_dictionary}).
...
This recognition leads naturally to \hyperref[sec:bk1_axiomata_prima]{Axiomata Prima}: \emph{Existence is not.} That is, existence is not self-justifying, but emergent from differentiation within its absence. It is not a substance or essence, but an effect discernible by the operation of opposing principles --- a regulated departure from uniformity.
Building on formal considerations explored elsewhere (\hyperref[sec:appE_directed_abstracts]{Appendix~E}, p.~\pageref{sec:appE_directed_abstracts}), we argue that any reality consistent with fundamental principles of observation and physicality must exhibit two primordial dynamics (\hyperref[sec:appC_dual_horizon]{Appendix~C}, p.~\pageref{sec:appC_dual_horizon}):
\begin{itemize}
...
\begin{tcolorbox}[
    ...
    left=8pt, right=8pt, top=1pt, bottom=1pt,
    boxrule=1pt,
    arc=3pt
]
\label{box:formalism_note}
\noindent\textbf{Notation Anchor.} For immediate resolution of technical symbols and operators, see \hyperref[sec:appA_symbol_dictionary]{Appendix~A: Symbol Dictionary} (p.~\pageref{sec:appA_symbol_dictionary}); definitions there are authoritative across all books.
...
Formal validation of this reflexive structure is developed in \hyperref[sec:appB_overview]{Appendix~B} (p.~\pageref{sec:appB_overview}).
...
This projection defines the symbolic substrate of reality as a function of perception---where coherence, curvature, and evolution arise from bounded interaction with a configuration manifold (\hyperref[sec:appD_preamble_nature_of_appendix]{Appendix~D}, p.~\pageref{sec:appD_preamble_nature_of_appendix}).
