...
\leavevmode
The three regimes and their boundary are the MAD--MAP--MAS band (Def.~\ref{definition:bk5_map_mad_mas_band}), established spectrally in the trichotomy theorem (Thm.~\ref{theorem:bk5_map_mad_mas_trichotomy}), with the free-energy derivative signs that select them computed in Thm.~\ref{theorem:bk5_enhanced_map_mad_duality_pr}. Case (i) ($\|\mathbb{R}_{AB}\|>\kappa_{crit}$, $\Omega_{AB}>0$) is the MAP regime: the coupling spectrum is real with dominant eigenvalue $\varphi$ and $\lim_{n}F_s(\Membrane_A^{(n)}\leftrightarrow\Membrane_B^{(n)})>0$. Case (ii) ($\|\mathbb{R}_{AB}\|>\kappa_{crit}$, $\Omega_{AB}<0$) is MAD: the antagonistic covenant makes the spectrum complex and $\lim_{n}F_s(\Membrane_A^{(n)}\cup\Membrane_B^{(n)})=0$ with collapse rate $\propto|\Omega_{AB}|$. Case (iii) ($\|\mathbb{R}_{AB}\|<\kappa_{crit}$) is the decoupling limit, $F_s\to F_s(\Membrane_A^{(n)})+F_s(\Membrane_B^{(n)})-\epsilon_n$ with $\epsilon_n\to0$. The regimes meet on the symbolic bifurcation manifold $\mathcal{B}=\{\Lambda_{AB}=1\}$ (Def.~\ref{definition:bk5_symbolic_bifurcation_man}), characterized by entropy inflection and critical symbolic temperature --- the asserted bifurcation locus.
\end{proof}
...
The variational comparison is made inside the process free-energy landscape:
operator paths are assumed to have costs bounded from below by
Def.~\ref{definition:bk5_process_free_energy}, while selection pressure is
supplied by Ax.~\ref{axiom:bk5_srmf_operator_selection_evolution}.
...
\leavevmode
The case computations above are organized by the MAD--MAP--MAS trichotomy (Thm.~\ref{theorem:bk5_map_mad_mas_trichotomy}) read through the process free-energy landscape (Def.~\ref{definition:bk5_process_free_energy}) under SRMF selection (Ax.~\ref{axiom:bk5_srmf_operator_selection_evolution}). Case (i) ($\Omega_{AB}>0$, strong coupling): the reflection terms exceed entropy production, $\frac{d}{ds}F_s(\Membrane_A\leftrightarrow\Membrane_B)>0$, so the dyad climbs to a bounded positive limit --- the real-$\varphi$ MAP regime. Case (ii) ($\Omega_{AB}<0$): the antagonistic cross-reflections amplify rather than damp drift, $\frac{d}{ds}F_s(\Membrane_A\cup\Membrane_B)<0$, driving $F_s\to0$ at rate $\propto|\Omega_{AB}|$ --- the complex-spectrum MAD regime. Case (iii) ($\|\mathbb{R}_{AB}\|<\kappa_{crit}$): sub-threshold coupling cannot sustain a covenant, so the membranes decouple and drift toward the zero-sum default --- the weak-coupling road into MAD --- with interaction term $\epsilon_n\to0$. The three cases are mutually exclusive and exhaustive in $(\|\mathbb{R}_{AB}\|,\Omega_{AB})$, completing the duality.
\end{proof}
...
\leavevmode
By the SRMF operator-selection axiom (Ax.~\ref{axiom:bk5_srmf_operator_selection_evolution}) viable systems evolve operators by descending the process free energy, so in a parameter chart the operator obeys the gradient flow $\dot{\mathcal{O}}_t=-\nabla_{\mathcal{O}}\Fproc$. Writing $\Fproc=\mathcal{E}_{\text{cost}}-T_{\text{meta}}(\mathcal{E}_{\text{eff}}+\mathcal{C}_{\text{hint}})$ (Def.~\ref{definition:bk5_process_free_energy}), the descent direction is
\[
-\nabla_{\mathcal{O}}\Fproc=-\nabla_{\mathcal{O}}\mathcal{E}_{\text{cost}}+T_{\text{meta}}\big(\nabla_{\mathcal{O}}\mathcal{E}_{\text{eff}}+\nabla_{\mathcal{O}}\mathcal{C}_{\text{hint}}\big).
\]
\emph{(2)} This is exactly steepest descent in $\Fproc$, which is clause~(2). \emph{(1)} When $\mathcal{E}_{\text{eff}}[\mathcal{O}_t,S_t]<\theta_{\text{crit}}$, the precision-refinement coupling (Def.~\ref{definition:bk4_test_time_precision_refinement}) makes the effectiveness-restoring term $T_{\text{meta}}\nabla_{\mathcal{O}}\mathcal{E}_{\text{eff}}$ the dominant component of the velocity, with magnitude set by the shortfall; hence the operator adapts at a rate proportional to $\Delta\mathcal{E}_{\text{eff}}=\theta_{\text{crit}}-\mathcal{E}_{\text{eff}}$, which is clause~(1). \emph{(3)} Because the flow minimizes $\Fproc$ and not $\mathcal{E}_{\text{cost}}$ alone, restoring effectiveness can require a costlier operator; along such a segment $\dot{\mathcal{E}}_{\text{cost}}>0$ while $\dot{\Fproc}\le 0$, so $\mathcal{E}_{\text{cost}}$ may increase transiently, which is clause~(3).
\end{proof}
...
\leavevmode
Read operator complexity $\mathcal{C}(\mathcal{O})$ as the cost-bearing structure of the operator, $\mathcal{C}(\mathcal{O})\propto\mathcal{E}_{\text{cost}}[\mathcal{O}]$ (Def.~\ref{definition:bk5_process_free_energy}), and system stability $\mathcal{S}(S)$ as the sustained viability margin held against drift (the reserve keeping $\rho\in\viabilitydomain$, in the symbolic free-energy budget of Def.~\ref{definition:bk2_symbolic_free_energy}). Holding a complex operator \emph{stable} is what costs metabolic work: each unit of complexity must be actively maintained against entropic drift to the degree set by the stability margin, so the joint maintenance expenditure scales with the product, $E_{\text{maint}}=\mathcal{C}(\mathcal{O})\,\mathcal{S}(S)/\alpha$ for a conversion constant $\alpha>0$. By Operators Evolve (Prop.~\ref{proposition:bk5_operators_evolve}) sustained viability requires this expenditure to be fundable, $E_{\text{maint}}\le\MC(S)$ (Def.~\ref{definition:bk5_metabolic_capacity_mc_}, Prop.~\ref{proposition:bk5_fixed_metabolic_capacity}). Combining the two relations,
\[
...
\]
Admissible complexity is therefore metabolically budgeted: at fixed capacity, greater stability can be purchased only by reducing complexity, and conversely. The proof rests on the explicit structural reading of $\mathcal{C}$, $\mathcal{S}$, and the product-form maintenance cost; it is not an empirically fitted scaling law.
\end{proof}
...
\leavevmode
Reflective fidelity is limited by attainable recursion depth: each additional level of recursive reflection resolves finer symbolic structure, so $\mathcal{F}(\mathcal{O}_{\text{reflect}})$ is non-decreasing in the attainable depth $n$ and, with each level contributing a bounded marginal gain, is at most affine in it, $\mathcal{F}(\mathcal{O}_{\text{reflect}})\le \beta_0\,n_{\max}$ for a constant $\beta_0>0$. By the metabolically bounded reflection corollary (Cor.~\ref{corollary:bk5__metabolically_bounded_reflection_corollary}) the attainable depth is itself capped by capacity, $n_{\max}\le\log_k(\MC(S)/c_0+1)$, with the operative budget being the metabolic capacity (Prop.~\ref{proposition:bk5_fixed_metabolic_capacity}, Prop.~\ref{proposition:bk5_metabolic_capacity_non_decreasing}) that also caps the complexity available to the reflective operator (Cor.~\ref{corollary:bk5_complexity_stability_tradeoff}, in the free-energy budget of Def.~\ref{definition:bk2_symbolic_free_energy}). Combining the two bounds,
\[
\mathcal{F}(\mathcal{O}_{\text{reflect}})\le\beta_0\,\log_k\!\Big(\tfrac{\MC(S)}{c_0}+1\Big)=\frac{\beta_0}{\ln k}\,\ln\!\Big(\tfrac{\MC(S)}{c_0}+1\Big).
\]
Since $\ln(\MC(S)/c_0+1)\le \max(1,1/c_0)\,\ln(1+\MC(S))$ for all $\MC(S)>0$, setting $\beta:=(\beta_0/\ln k)\,\max(1,1/c_0)$ yields $\mathcal{F}(\mathcal{O}_{\text{reflect}})\le\beta\,\log(1+\MC(S))$. Reflective accuracy therefore grows at most logarithmically in metabolic capacity.
\end{proof}
...
\subsection{The MAD--MAP--MAS Band}
\label{subsec:bk5_map_mad_mas_band}
The Golden Ratio has now been established as the spectral invariant of sustainable symbolic life (Thm.~\ref{theorem:bk5_golden_ratio_spectral_invariant}, Cor.~\ref{corollary:bk5_symbolic_eigenlife}). We close Book~V by reading the dyadic covenant of Thm.~\ref{theorem:bk5_enhanced_map_mad_duality} as a \emph{band} with $\varphi$ at its sustainable centre, flanked by two opposite failure edges.
...
\begin{theorem}[The MAD--MAP--MAS Trichotomy]
